...
%>
<jsp:useBean id="adminBean" class="minuhy.xiaoxiang.blog.bean.admin.AdminBlogBean"></jsp:useBean>
<jsp:useBean id="searchBean" class="minuhy.xiaoxiang.blog.bean.admin.AdminBlogSearchBean"></jsp:useBean>
<%
    // 获取页面参数
    int pageNumber;
	...
	<div style="text-align: center;padding: 10px;width: 1500px;margin: 0 auto;">
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th>编号</th>
                    <th>有效</th>
                    <th class="can-search">作者编号</th>
                    <th class="can-search">标题</th>
                    <th class="can-search">内容</th>
                    <th>阅读量</th>
                    <th>点赞量</th>
                    <th>评论数</th>
                    <th class="can-search">发布时间</th>
            ...
            <tbody>
                ...
                <tr id="tr<%=entity.getId()%>">
                    ...
                    <td><%=entity.getAuthorId()%></td>
                    <td><a title="<%=entity.getTitle()%>" href="<%=currentPath%>/read.jsp?i=<%=entity.getId()%>"><%= TextUtil.maxLenJustify(entity.getTitle(), 24) %></a></td>
                    <td><a href="<%=currentPath%>/post.jsp?ei=<%=entity.getId()%>">编辑</a></td>
                    <td><%=entity.getReadCount()%></td>
                    <td><%=entity.getLikeCount()%></td>
                    <td><%=entity.getCommentCount()%></td>
                ...
                <%
                        ...
                        // 数据库出错
						response.setHeader("refresh","3;"+currentPath + "/admin/blog.jsp");
                %>
                <tr>
        ...
        <%
                ...
                if(searchBean.getKeyWord()!=null){
                    ...
                    paginationBean.setTargetPage(currentPath+"/admin/blog.jsp?keyword="+searchBean.getKeyWord());
                }else{
                    paginationBean = adminBean.getPaginationBean();
                    paginationBean.setParamName("p");
                    paginationBean.setTargetPage(currentPath+"/admin/blog.jsp");
                }
        %>
        <div id="page-nav">
        ...
        <div style="padding: 10px;">
	        <form style="display: inline-block;margin: 20px;" action="operate.jsp" method="post">
	        	...
	        	<input name="type" value="1" type="hidden" required="required" > <!-- 表示的是操作的类型是用户 -->
	        ...
	        <form onsubmit="return sumbit_sure()"  style="display: inline-block;margin: 20px;" action="operate.jsp" method="post">
	        	...
	        	<input name="type" value="1"  type="hidden" required="required" >
...
<script>
    ...
    function optionItem(id) {
        console.log('操作编号：'+id)
        window.location.href="<%=currentPath%>/admin/edit.jsp?t=1&p=<%=pageNumber%>&i="+id;
    }
