...
%>
<jsp:useBean id="searchBean" class="minuhy.xiaoxiang.blog.bean.admin.AdminUserSearchBean"></jsp:useBean>
<jsp:useBean id="adminBean" class="minuhy.xiaoxiang.blog.bean.admin.AdminUserBean"></jsp:useBean>
<%
    // 获取页面参数
    int pageNumber;
	...
	<div style="text-align: center;padding: 10px;width: 1500px;margin: 0 auto;">
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th>编号</th>
                    <th>激活</th>
                    <th class="can-search">账号</th>
                    <th>性别</th>
                    <th>权限</th>
                    <th class="can-search">昵称</th>
                    <th>博文</th>
                    <th>被点赞</th>
                    <th>被阅读</th>
                    <th class="can-search">注册时间</th>
                    <th class="can-search">登录时间</th>
                    <th class="can-search">登录IP</th>
                    <th>操作</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr id="tr<%=entity.getId()%>">
                    ...
                    %></td>
                    <td><%=entity.getRole()==1?"管理员":"用户"%></td>
                    <td><%=entity.getNick()%></td>
                    <td><%=entity.getBlogCount()%></td>
                    <td><%=entity.getBlogLikeCount()%></td>
                    <td><%=entity.getBlogReadCount()%></td>
                    <td><%=TimeUtil.timestamp2DateTime(entity.getCreateTimestamp())%></td>
                    <td><%=TimeUtil.timestamp2DateTime(entity.getLastLoginTimestamp())%></td>
                    <td><%=entity.getLastLoginIp()%></td>
                    <td>
                ...
                <%
                            ...
                            // 无数据
							response.setHeader("refresh","3;"+currentPath + "/admin/user.jsp");
                %>
                <tr>
		...
		<%
                ...
                if(searchBean.getKeyWord()!=null){
                    ...
                    paginationBean.setTargetPage(currentPath+"/admin/user.jsp?keyword="+searchBean.getKeyWord());
                }else{
                    paginationBean = adminBean.getPaginationBean();
                    paginationBean.setParamName("p");
                    paginationBean.setTargetPage(currentPath+"/admin/user.jsp");
                }
        %>
        <div id="page-nav">
        ...
        <div style="padding: 10px;">
	        <form style="display: inline-block;margin: 20px;" action="operate.jsp" method="post">
	        	...
	        	<input name="type" value="0" type="hidden" required="required" > <!-- 表示的是操作的类型是用户 -->
	        ...
	        <form onsubmit="return sumbit_sure()"  style="display: inline-block;margin: 20px;" action="operate.jsp" method="post">
	        	...
	        	<input name="p" value="<%= pageNumber %>"  type="hidden" required="required" >
	        	<input name="type" value="0"  type="hidden" required="required" >
...
<script>
    ...
    function optionItem(id) {
        console.log('操作编号：'+id)
        window.location.href="<%=currentPath%>/admin/edit.jsp?t=0&p=<%=pageNumber%>&i="+id;
    }
